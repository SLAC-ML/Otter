...
def data_source():
    """Fixture: Create and validate BadgerArchiveDataSource"""
    ...
    # Health check
    assert ds.health_check(), "Data source health check failed"

    return ds


@pytest.fixture(scope="module")
def run_path(data_source):
    """Fixture: Get a valid run path from the archive"""
    runs = data_source.list_runs(limit=1)
    assert runs, "No runs found in archive"
    return runs[0]


@pytest.fixture(scope="module")
def metadata(data_source, run_path):
    """Fixture: Load metadata for the selected run"""
    meta = data_source.load_run_metadata(run_path)
...
def test_data_source(data_source):
    """Test BadgerArchiveDataSource initialization and health check"""
    print("=" * 60)
    print("TEST 1: BadgerArchiveDataSource Initialization and Health Check")
    print("=" * 60)

    # The fixture already validates initialization and health check
    # Here we just verify the object exists
    assert data_source is not None
    assert isinstance(data_source, BadgerArchiveDataSource)
...
def test_list_runs(data_source):
    ...
    assert len(runs) > 0, "Expected at least 1 run"
...
def test_load_metadata(data_source, run_path, metadata):
    ...
    # The fixture already loaded metadata, we just verify it
    assert metadata is not None
    assert "name" in metadata
    assert "beamline" in metadata
...
def test_context_creation(metadata, run_path):
    ...
    assert access_details is not None
    assert "run_identification" in access_details
    assert "optimization_config" in access_details
...
# ============================================================================
# Manual Test Runner (preserves original behavior)
# ============================================================================


def main():
    """Run all tests manually (preserves original custom output)"""
    print("\n" + "=" * 60)
    ...
    # Initialize data source
    archive_path = "/Users/zhezhang/Desktop/Otter/data/archive"
    print(f"\nInitializing data source with archive: {archive_path}")
    ...
    runs = data_source.list_runs(limit=1)
    if not runs:
        print("\n✗ No runs found - aborting")
        return False

    run_path = runs[0]
    print(f"✓ Found run: {run_path}")
