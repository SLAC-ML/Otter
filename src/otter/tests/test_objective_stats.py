...
def test_objective_stats():
    ...
    required_fields = ["initial_values", "min_values", "max_values", "final_values"]
        ...
        if metadata["initial_values"]:
            ...
            if init_val is not None and final_val is not None:
                if direction == "MAXIMIZE":
                    ...
                    improvement = (
                        ((final_val - init_val) / abs(init_val)) * 100 if init_val != 0 else 0
                    )
                    best_improvement = (
                        ((max_val - init_val) / abs(init_val)) * 100 if init_val != 0 else 0
                    )
                    ...
                    improvement = (
                        ((init_val - final_val) / abs(init_val)) * 100 if init_val != 0 else 0
                    )
                    best_improvement = (
                        ((init_val - min_val) / abs(init_val)) * 100 if init_val != 0 else 0
                    )
                    print(f"  Improvement (init→final): {improvement:.1f}%")
